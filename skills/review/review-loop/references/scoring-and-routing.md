...
**Baseline smell findings (Agent #1) are always ask-routed.** Agent #1's documented-guidance findings score and route normally. Its two *baseline* heuristics — possible Mysterious Name, possible Duplicated Code, which fire even where the repo documents nothing — are judgement calls by construction, so they are surfaced as proposals and **never auto-applied**, whatever the score. Auto-applying a rename or an extraction off a heuristic is the failure mode this prevents.

**Spec-conformance findings (Agent #11) are not scored at all.** They skip this rubric and Step 6 entirely. Each one is anchored to a quoted line of a written spec, so its severity is settled by its class (MISSING / UNASKED / WRONG), not by a confidence estimate — and scoring them on the same 0-100 scale as #1/#2/#4 would re-merge the very axis the agent exists to keep separate, letting a naming nit outrank a missed requirement. They are **always routed to ask-user**, never auto-applied, and reported in their own section (see `report-format.md`). The Dismissed-list check still applies.
