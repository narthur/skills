...
With no `--browser`, `open` uses `$PLAYWRIGHT_MCP_BROWSER`. `~/.zshrc` sets it to `firefox` when Google Chrome isn't installed, so a bare `open` works either way. If `open` still says Chrome is missing, the shell didn't load `~/.zshrc`: pass `--browser firefox`.
